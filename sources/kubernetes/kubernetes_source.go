...
import (
	...
	metrics "github.com/changqings/kube-eventer/metrics/prometheus"

	"github.com/prometheus/client_golang/prometheus"

	"github.com/changqings/kube-eventer/common/kubernetes"
...
func (ks *KubernetesEventSource) GetNewEvents() *core.EventBatch {
	startTime := time.Now()
	defer func() {
...
event_loop:
	for {
		select {
		case event := <-ks.localEventsBuffer:
...
func (ks *KubernetesEventSource) watch() {
	// Outer loop, for reconnections.
	for {
		events, err := ks.eventClient.List(context.Background(), metav1.ListOptions{Limit: 1})
		if err != nil {
		...
		watcher, err := ks.eventClient.Watch(
			context.Background(),
			metav1.ListOptions{
				Watch:           true,
			...
			case watchUpdate, ok := <-watchChannel:
				...
				if event, ok := watchUpdate.Object.(*kubeapi.Event); ok {
					...
					case kubewatch.Added, kubewatch.Modified:
						...
						select {
						case ks.localEventsBuffer <- event:
							// Ok, buffer not full.
						default:
