...
PREFIX?=registry.aliyuncs.com/xx
FLAGS=
ARCH?=amd64
...
KUBE_EVENTER_LDFLAGS=-w -X github.com/changqings/kube-eventer/version.Version=$(VERSION) -X github.com/changqings/kube-eventer/version.GitCommit=$(GIT_COMMIT)
...
build: clean
	go mod tidy
	GOARCH=$(ARCH) CGO_ENABLED=0 go build -ldflags "$(KUBE_EVENTER_LDFLAGS)" -o kube-eventer  main.go
